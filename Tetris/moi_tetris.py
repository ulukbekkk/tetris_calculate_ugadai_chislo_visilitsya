...
game_sc = Canvas(tk, width=W*TILE +1, height=H * TILE +1, bg="red", highlightthickness=0)
game_sc.place(x=20, y=20, anchor=NW)

img_obj1 = PhotoImage(file="my_img/bg.png")
sc.create_image(0, 0, anchor=NW, image=img_obj1)

img_obj2 = PhotoImage(file="my_img/bg2.png")
...
# создаем текс и шрифт и цвет для игры
sc.create_text(505 , 30, text="TETRIS", font=("WiGuru 2", 45),fill="red", anchor= NW)
sc.create_text(535, 780, text="score:", font=("WiGuru 2", 40),fill="white", anchor= NW)
_score = sc.create_text(550, 840, text=str(score), font=("WiGuru 2", 40),fill="white", anchor= NW)
sc.create_text(525, 650, text="record:", font=("WiGuru 2", 40),fill="white", anchor= NW)
_record = sc.create_text(550, 710 , text=record, font=("WiGuru 2", 40),fill="gold", anchor= NW)
...
while app_running:
    if app_running:
        ...
        fig = []
        #Падающая фигура внутри внутреннего пространства
        for i in range(4):
            figure_rect_x = figure[i][0] * TILE
            figure_rect_y = figure[i][1] * TILE
            fig.append(game_sc.create_rectangle(figure_rect_x, figure_rect_y,figure_rect_x + TILE, figure_rect_y + TILE, 
                                                                                        fill=get_to_hex(color)))
        ...
        fig2 =[]
        # #Следующая фигура которая выйдет на рисованная в глобал интерфейсе
        for i in range(4):
            ...
            fig2.append(sc.create_rectangle(figure_rect_x, figure_rect_y,figure_rect_x + TILE, figure_rect_y + TILE, 
                                                                                fill=get_to_hex(next_color)))


        # draw title
        sc.itemconfig(_score, text=str(score))
        sc.itemconfig(_record, text=record)
        ...
        for i in range(W):
            if field[0][i]:
                ...
                score = 0
                for item in grid:
                    game_sc.itemconfigure(item, fill=get_to_hex(get_color()))
                    time.sleep(0.005)
                    tk.update_idletasks()
                    tk.update()
                for item in grid:
